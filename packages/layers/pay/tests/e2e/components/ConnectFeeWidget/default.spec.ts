...
test.describe('Connect Fee (default)', () => {
  ...
  test('Loads with expected visuals', async ({ page }) => {
    ...
    await expect(page.getByTestId('service-fee')).toBeVisible()
    await expect(page.getByTestId('service-fee')).toContainText('Service Fee')
    await expect(page.getByTestId('service-fee')).toContainText('$1.50')
  })
