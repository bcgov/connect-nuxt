...
test.describe('Connect Button Control (default)', () => {
  test('Loads with expected visuals', async ({ page }) => {
    ...
    await expect(leftButtons.getByRole('link', { name: 'Left 3 Link' })).toBeVisible()
    const rightButtons = buttonControl.getByTestId('right-buttons')
    await expect(rightButtons).toBeVisible()
    await expect(rightButtons.getByRole('button', { name: 'Right 1' })).toBeVisible()
    await expect(rightButtons.getByRole('button', { name: 'Right 2' })).toBeVisible()
  })
  test('Loading mechanism works as expected', async ({ page }) => {
    ...
    const leftButton3 = leftButtons.getByRole('link', { name: 'Left 3 Link' })
    const rightButton1 = rightButtons.getByRole('button', { name: 'Right 1' })
    const rightButton2 = rightButtons.getByRole('button', { name: 'Right 2' })
    ...
    expect(leftButton3).toBeDisabled()
